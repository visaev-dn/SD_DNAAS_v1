...
class BridgeDomainConsolidationManager:
    ...
    def _extract_primary_vlan(self, bridge_domain_config) -> Optional[int]:
        ...
        if hasattr(bridge_domain_config, 'vlan_list') and bridge_domain_config.vlan_list:
            return bridge_domain_config.vlan_list[0] if bridge_domain_config.vlan_list else None
    ...
    def _merge_topologies(self, group_topologies: List[TopologyData], 
        ...
        consolidated_topology = TopologyData(
            bridge_domain_name=base_topology.bridge_domain_name,
            topology_type=base_topology.topology_type,
            vlan_id=base_topology.vlan_id,
            discovered_at=base_topology.discovered_at,
            scan_method=base_topology.scan_method,
            devices=base_topology.devices,
            interfaces=base_topology.interfaces,
            paths=base_topology.paths,
            bridge_domain_config=base_topology.bridge_domain_config,
            validation_status=base_topology.validation_status,
            confidence_score=base_topology.confidence_score
        )
        
        return consolidated_topology
    
    def _select_best_topology(self, group_topologies: List[TopologyData]) -> TopologyData:
        """Select the best topology as the base for consolidation"""
        
        # Simple selection: prefer higher confidence scores
        best_topology = group_topologies[0]
        best_confidence = best_topology.confidence_score or 0.0
        
        for topology in group_topologies[1:]:
            current_confidence = topology.confidence_score or 0.0
    ...
    def _print_consolidation_decision(self, decision_result: ConsolidationDecisionResult):
        """Print consolidation decision details"""
        
        print(f"🔍 Consolidation Decision: {decision_result.decision.value.upper()}")
        print(f"   Key: {decision_result.consolidation_key}")
        ...
        if decision_result.approval_reasons:
            print(f"   ✅ Reasons: {', '.join(decision_result.approval_reasons)}")
        
        if decision_result.rejection_reasons:
            print(f"   ❌ Reasons: {', '.join(decision_result.rejection_reasons)}")
        
        if decision_result.warnings:
            print(f"   ⚠️  Warnings: {', '.join(decision_result.warnings)}")
    ...
    def _print_consolidation_summary(self):
        ...
        print("📊 CONSOLIDATION SUMMARY")
        print("=" * 40)
        print(f"Total Groups: {self.consolidation_stats['total_groups']}")
        print(f"Consolidated: {self.consolidation_stats['consolidated_groups']}")
        print(f"Rejected: {self.consolidation_stats['rejected_groups']}")
        print(f"Review Required: {self.consolidation_stats['review_required_groups']}")
        
        if self.consolidation_stats['total_groups'] > 0:
            success_rate = (self.consolidation_stats['consolidated_groups'] / 
                          self.consolidation_stats['total_groups']) * 100
            print(f"Success Rate: {success_rate:.1f}%")
        
        print()
    
    def get_consolidation_candidates(self, topologies: List[TopologyData]) -> Dict[str, List[str]]:
        """Get potential consolidation candidates grouped by consolidation key"""
        ...
        for topology in topologies:
            try:
                consolidation_key = self._generate_consolidation_key(topology)
                consolidation_candidates[consolidation_key].append(topology.bridge_domain_name)
            except ValueError:
                # Skip topologies that can't be consolidated
                continue
        
        # Filter to only groups with multiple bridge domains
        return {key: names for key, names in consolidation_candidates.items() if len(names) > 1}
